...
BELA_PRE_ENABLE_STARTUP_COMMAND=printf "ACTIVE=1\nPROJECT=$(PROJECT)\nARGS=$(COMMAND_LINE_OPTIONS)" > $(BELA_STARTUP_ENV)
BELA_PRE_DISABLE_STARTUP_COMMAND=printf "ACTIVE=0" > $(BELA_STARTUP_ENV)

ifeq ($(DEBIAN_VERSION), stretch)
BELA_ENABLE_STARTUP_COMMAND=$(BELA_PRE_ENABLE_STARTUP_COMMAND) && systemctl enable bela_startup
