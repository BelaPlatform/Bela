...
unsigned int capTouchSleepIntervalUs = 30000; 
void readCapSensorLoop(void*)
{
	while(!gShouldStop)
	{
		if(capTouch.ready()) {
			capTouch.readI2C();
			//for(unsigned int i = 0; i < sizeof(capTouch.rawData)/sizeof(int); i++)
				//printf("%5d ", capTouch.rawData[i]);
		}
...
{
// MODIFICATION START
	libpd_start_message(capTouch.numSensors);
	for(unsigned int n = 0; n < capTouch.numSensors; ++n)
	{
		libpd_add_float(capTouch.rawData[n]/(4096.f));
		// libpd_add_float(capTouch.rawData[n]);
	}
	libpd_finish_message("bela_capTouch", "list");
//MODIFICATION END
	int num;
#ifdef PARSE_MIDI
